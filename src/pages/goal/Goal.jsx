  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        toast.error(`Failed to delete goal: ${error.response?.data.message || error.message}`);
      }
    }
  };
    ...
    <>
      ...
      <div className="flex my-10">
        <div className="h-auto w-[200px] rounded-lg border-2 border-gray-300 p-4">
          <div className="font-extrabold uppercase">Rs.{totalGoal.toFixed(2)}</div>
          <div className="font-bold uppercase text-gray-600 text-[14px]">Total goal</div>
        </div>
      </div>
              ...
              filteredGoal.map((item) => (
                <tr key={item._id} className="border-b border-gray-200 bg-white text-gray-900">
                  ...
                  <td className="px-6 py-4">
                    {item.status ? item.status : "Pending"} {/* Default to "Pending" if no status */}
                  </td>
